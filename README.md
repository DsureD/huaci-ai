...
- `huaci-ai_x.x.x_x64_en-US.msi` - Windows 安装包（推荐）
...
- **便携模式**：程序目录下的 `config.json`（优先）
- **标准模式**：`C:\Users\你的用户名\AppData\Roaming\huaci-ai\config.json`

编辑 `config.json`，配置你的 API 接口：

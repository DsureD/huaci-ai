...
// 钩子线程向工作线程发送 (按下点, 抬起点) 的通道
static SIGNAL_TX: Mutex<Option<Sender<(Option<(i32, i32)>, (i32, i32))>>> = Mutex::new(None);
...
pub fn install_mouse_hook(app_handle: AppHandle) -> anyhow::Result<()> {
    let (tx, rx) = mpsc::channel::<(Option<(i32, i32)>, (i32, i32))>();
    *SIGNAL_TX.lock().unwrap() = Some(tx);

    // 工作线程：仅“拖选”或“双击”才视为有效选词动作，普通单击不取词
    let worker_app = app_handle.clone();
    std::thread::spawn(move || {
        // 上一次“抬起”的时间与位置，用于识别双击选词
        let mut last_up: Option<(Instant, i32, i32)> = None;

        while let Ok((down, up)) = rx.recv() {
            ...
            let is_drag = match down {
                Some((dx, dy)) => {
                    (up.0 - dx).abs() >= DRAG_THRESHOLD || (up.1 - dy).abs() >= DRAG_THRESHOLD
                }
                None => false,
            };
